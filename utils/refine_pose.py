...
def refine_pose(
    model_path, sample, pred_r, pred_t, num_iters=50, lr=0.0005, verbose=False
):
    ...
    refined_r, refined_t, losses = rf.refine_pose(
        R, T, gt_depth.cuda(), num_iters=num_iters, lr=lr, verbose=verbose
    )
    return (
        refined_r.cpu().detach().squeeze().numpy(),
        refined_t.cpu().detach() * 1000,
        losses,
    )
