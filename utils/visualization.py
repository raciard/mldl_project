...
def load_models_info():
    ...
    obj_path = os.path.join(
        "datasets/Linemod_preprocessed", "models", "models_info.yml"
    )
...
def visualize_3d_bbox(
    ...
    gt_R = gt_R.astype(np.float64) if gt_R is not None else None
    gt_t = gt_t.astype(np.float64) if gt_t is not None else None
    camera_matrix = (
    ...
    if pred_R is not None and pred_t is not None and camera_matrix is not None:
        pred_points_2d, _ = cv2.projectPoints(
            points_3d, pred_R, pred_t, camera_matrix, None
        )
